...
class CRC32(object):
	...
	crc32_tab = [0x00000000, 0x77073096, 0xee0e612c,
	...
	def crc32(self, bytes, crc=0):
		...
		for i in range(count):  
			...
			crc = ((crc >> 8) & 0x00FFFFFF) ^ self.crc32_tab[(crc ^ ord( bytes[i])) & 0xFF]
		return crc ^ 0xffffffff
		
	def filecrc32(self, filepath):
		with open(filepath,'rb') as f:
			return self.crc32(f.read())

class MediaScriptHeader(object):
	"""
	the header of binary script file:
	struct MediaScriptHeader {
		int header_len;
		int script_len;
		uint script_crc32;
	};
	...
	def __init__(self, script_file=None, machine="None"):
		self.header = None
		if script_file:
			self.GetHeader(script_file, machine)
			
	def __str__(self):
		return "headerlen=%d, scriptLen=%d, CRC32=0x%X machine=%s" % (struct.unpack("<3I20s", self.header))
	...
	def GetHeader(self, script_file=None, machine="none"):
		if not script_file:
			return self.header
		...
		with open(script_file, 'rb') as f:
			bytes = f.read()
			script_len = len(bytes)
			script_crc32 = crc32.crc32(bytes, script_crc32)

		self.header = struct.pack('<3I20s', self.header_len,script_len,script_crc32,machine)	
		return self.header

class MediaScript(object):
	"""
	write header information into binary script file
	"""
	
	def tobin(self, script_file, binfile="media.bin", machine="none"):
		header = MediaScriptHeader(script_file, machine)
		print(str(header))
		with open(binfile, 'wb') as dst:
			header.toFile(dst)
			with open(script_file, 'rb') as f:
...
if __name__=='__main__':
	...
	machine = sys.argv[3]
	MediaScript().tobin(script_file, target_file, machine)